...
export default function GlobalError({ error, reset }: ErrorProps) {
  useEffect(() => {
    // Log the error to the console in development for quick inspection.
        ...
        <div className="animate-fade-up mx-auto max-w-2xl space-y-8 text-center">
          ...
          <div className="relative mx-auto inline-flex items-center justify-center">
            ...
            <div className="animate-float relative flex h-28 w-28 items-center justify-center rounded-3xl border border-white/60 bg-white/70 shadow-[0_8px_32px_rgba(7,156,251,0.15)] backdrop-blur-md md:h-32 md:w-32">
              <ServerCrash className="h-14 w-14 text-[#079cfb] md:h-16 md:w-16" strokeWidth={1.5} />
            </div>
          </div>
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-white/60 bg-white/70 px-4 py-1.5 shadow-sm backdrop-blur-sm">
            <span className="text-xs font-semibold tracking-widest text-[#037ecc]">
              ALGO NO ANDA BIEN
            </span>
          </div>
          ...
          <h1 className="font-serif text-4xl leading-tight font-bold text-balance sm:text-5xl md:text-6xl">
            ...
            <span className="gradient-text">salió mal</span>
          </h1>

          {/* Subtitle */}
          <p className="mx-auto max-w-lg text-base leading-relaxed text-[#334155] opacity-90 md:text-lg">
            Tuvimos un inconveniente procesando tu solicitud. Podés reintentar o volver al inicio —
            nuestro equipo ya fue notificado.
          </p>

          {/* Dev-only error detail */}
          {isDev && error.message ? (
            <div className="mx-auto max-w-lg rounded-xl border border-white/60 bg-white/60 p-4 text-left shadow-sm backdrop-blur-sm">
              <p className="mb-1 text-xs font-semibold tracking-wide text-[#037ecc]">
                DETALLE (SOLO DEV)
              </p>
              <p className="font-mono text-sm break-words text-[#334155]">{error.message}</p>
              {error.digest ? (
                <p className="mt-2 font-mono text-xs text-[#6b7280]">digest: {error.digest}</p>
              ) : null}
            </div>
          ) : null}
          ...
          <div className="flex flex-col justify-center gap-3 pt-4 sm:flex-row md:gap-4">
            ...
            >
              <Link href="/">
                <Home className="mr-2 h-5 w-5 text-[#079cfb] transition-transform group-hover:rotate-12" />
                Volver al inicio
              </Link>
            </Button>
          </div>
